...
function formatBladeStringInlineEchoWithPint(content) {
    const options = optionDiscovery_1.defaultSettings;
    return (0, utils_1.formatBladeStringWithPint)(content, {
        ...options,
        echoStyle: 'inline'
    });
}
suite('Pint Transformer: Inline Echo Blocks', () => {
    test('pint: it formats valid PHP code', () => {
        assert_1.default.strictEqual(formatBladeStringInlineEchoWithPint('{{$test   +$that}}').trim(), '{{ $test + $that }}');
    });
    ...
    test('pint: it formats php without crashing', () => {
        ...
        assert_1.default.strictEqual(formatBladeStringInlineEchoWithPint(input).trim(), out);
    });
    test('pint: it ignores invalid PHP code in {{{', () => {
        assert_1.default.strictEqual(formatBladeStringInlineEchoWithPint('{{{$test   $+++$that}}}').trim(), '{{{ $test   $+++$that }}}');
    });
...
>
    Some link
        ...
        assert_1.default.strictEqual(formatBladeStringInlineEchoWithPint(input).trim(), expected);
    });
    test('pint: it can format inline echos as text', () => {
        assert_1.default.strictEqual(formatBladeStringInlineEchoWithPint(`
            <p>test {{ $title }} test
...
></label>
`;
        assert_1.default.strictEqual(formatBladeStringInlineEchoWithPint(input), out);
    });
...
`;
        assert_1.default.strictEqual(formatBladeStringInlineEchoWithPint(input), out);
        assert_1.default.strictEqual(formatBladeStringInlineEchoWithPint(out), out);
    });
...
></div>`;
        assert_1.default.strictEqual(formatBladeStringInlineEchoWithPint(template).trim(), out);
    });
...
`;
        assert_1.default.strictEqual(formatBladeStringInlineEchoWithPint(input), output);
    });
...
//# sourceMappingURL=formatter_pint_inline_echo_block.test.js.map