...
var reloadImportRegistries = new import_coc.RequestType0(
...
async function tryActivate(context) {
      ...
      provideDefinition: async (document, position, token, next) => {
        ...
        if (semver.satisfies(serverVersion, SERVER_SEMVER_1210)) {
          if (def && Array.isArray(def)) {
            def.map((d) => {
              if (import_coc3.LocationLink.is(d) && d.targetUri.startsWith("deno:asset")) {
  ...
  if (semver.valid(serverVersion) && !semver.satisfies(serverVersion, SERVER_SEMVER)) {
    import_coc3.window.showWarningMessage(
